import{f as y}from"./index-CIVIkReU.js";const H=["Melee","Melee armours","Ranged","Ranged armours","Magic","Magic armours","Prayer armours","Food","Other skill boosts","Potion","Teleports","Cooking","Construction","Crafting","Farming","Fletching","Fishing","Herblore","Hunter","Mining","Prayer","Runecraft","Smithing","Woodcutting"];function a(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function x(e){return String(e).toLowerCase().trim().replace(/[^a-z0-9]+/g,"-").replace(/(^-|-$)/g,"")}function M(e){return String(e||"").toLowerCase().replace(/[^a-z0-9]/g,"")}function O(e,n){if(!e)return null;const t=String(n||"").toLowerCase();return e.has(t)?e.get(t):null}function P(e,n){const t=e?.amount;if(typeof t=="number")return{value:t,known:!0};if(t&&typeof t=="object"&&t.type==="tiered"){const r=t.basedOn||e?.skill,c=O(n,r);if(c==null)return{value:null,known:!1};const o=Array.isArray(t.tiers)?t.tiers:[];for(const s of o)if(c>=s.min&&c<=s.max)return{value:s.amount,known:!0};return{value:null,known:!1}}return{value:null,known:!1}}async function R(){const e=y.obtained,n=y.rolled;if(!e||!n)return`
            <h1>Unlocks by skill</h1>
            <p>Please upload your files on the Upload page first.</p>
        `;await y.ensureItemsLoaded();const t=y.items||[],r=y.player?.levels||null,c=r?new Map(Object.entries(r).map(([d,m])=>[String(d).toLowerCase(),m])):null,o=new Set(e),s=new Set(n),l=new Map,u=H.map(d=>{const m=x(d)||"section",g=l.get(m)||0;l.set(m,g+1);const $=g?`${m}-${g+1}`:m;return{tag:d,id:$}}),p=u.map(({tag:d,id:m})=>`
        <a class="unlock-jump-link" href="#${a(m)}">${a(d)}</a>
    `).join(""),j=u.map(({tag:d,id:m})=>{const g=t.filter(i=>{const k=i.tags||[];return(Array.isArray(k)?k.includes(d):k===d)?o.has(i.id)&&s.has(i.id):!1});g.sort((i,k)=>i.name.localeCompare(k.name));const $=g.map(i=>{const k=Array.isArray(i.boosts)?i.boosts:[],w=k.length?`
