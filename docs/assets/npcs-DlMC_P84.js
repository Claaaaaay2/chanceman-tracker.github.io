import{f as e,N as R,i as $}from"./index-DmLeSzun.js";import{b as k,g as w,a as L,f as D}from"./npcDropEntries-D7raXqRD.js";function p(s){return String(s).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function P(s){return/^Reward casket \(/i.test(s||"")}async function T(){if(!e.player)return`
            <h1>NPC drops</h1>
            <p>Please upload your files and player name on the Upload page first.</p>
        `;await e.ensureItemsLoaded();const s=e.filters||{},o=!!s.npcCollapseDrops,{entries:r,rolledSet:c}=await k({items:e.items||[],obtained:e.obtained||[],rolled:e.rolled||[],player:e.player,filters:s}),i=r.map(t=>{const d=t.items.length,n=d===1?"item":"items",u=R[t.npcName]?.wiki,N=P(t.npcName)?'<span class="clue-step-info npc-drop-rate-info" tabindex="0" aria-label="Clue casket roll information" title="Chance for a new roll is per roll inside the casket, not per casket.">i</span>':"",S=u?`<a class="npc-drop-name-link" href="${u}" target="_blank" rel="noreferrer">${p(t.npcName)}</a>`:`<span class="npc-drop-name-text">${p(t.npcName)}</span>`;let y="";o||(y=t.items.sort((a,m)=>{const h=w(a.sources?.drops?.[t.npcName]),v=w(m.sources?.drops?.[t.npcName])-h;return v!==0?v:a.name.localeCompare(m.name)}).map(a=>{const m=a.sources?.drops?.[t.npcName],h=L(m),b=c.has(a.id);return`
                    <div class="npc-drop-item" onclick="navigate('/item?id=${a.id}')">
                        <img class="npc-drop-item-image" src="/images/${a.image}" alt="${p(a.name)}">
                        <span class="npc-drop-item-name">${p(a.name)}${p(h)}</span>
                        ${b?'<span class="badge rolled npc-drop-rolled">Rolled</span>':""}
                    </div>
                `}).join(""));const E=t.items.map(a=>a.name.toLowerCase()).join(" ");return`
    ...
    `}function C(s){const o=(e.filters?.npcSearch||"").trim().toLowerCase(),r=s.querySelectorAll(".npc-drop-card");let c=0;for(const l of r){if(!o){l.style.display="",c+=1;continue}const t=l.dataset.name||"",d=l.dataset.items||"",n=t.includes(o)||d.includes(o);l.style.display=n?"":"none",n&&(c+=1)}const i=document.getElementById("npcEmptyState");i&&(i.style.display=c?"none":"")}let f=null;function x(){I();const s=document.getElementById("npcDropList");s&&C(s);const o=document.getElementById("npcSortToggle");o&&(o.checked=!!e.filters?.npcSortByRate);const r=document.getElementById("npcOnlyRolledToggle");r&&(r.checked=!!e.filters?.npcOnlyRolled);const c=document.getElementById("npcCollapseDropsToggle");c&&(c.checked=!!e.filters?.npcCollapseDrops),$(()=>window.dispatchEvent(new PopStateEvent("popstate")));const i=async n=>{if(n.target.id!=="npcSearch")return;const u={...e.filters,npcSearch:n.target.value};await e.setFilters(u);const g=document.getElementById("npcDropList");g&&C(g)},l=async n=>{if(n.target.id!=="npcSortToggle")return;const u=!e.filters?.npcSortByRate;await e.setFilters({...e.filters,npcSortByRate:u}),window.dispatchEvent(new PopStateEvent("popstate"))},t=async n=>{n.target.id==="npcOnlyRolledToggle"&&(await e.setFilters({...e.filters,npcOnlyRolled:n.target.checked}),window.dispatchEvent(new PopStateEvent("popstate")))},d=async n=>{n.target.id==="npcCollapseDropsToggle"&&(await e.setFilters({...e.filters,npcCollapseDrops:n.target.checked}),window.dispatchEvent(new PopStateEvent("popstate")))};document.addEventListener("input",i),document.addEventListener("input",l),document.addEventListener("input",t),document.addEventListener("input",d),f=()=>{document.removeEventListener("input",i),document.removeEventListener("input",l),document.removeEventListener("input",t),document.removeEventListener("input",d)}}function I(){typeof f=="function"&&f(),f=null}export{T as default,x as init,I as teardown};
